var apikey = Ti.App.Properties.getString('GAPI_production');
...
exports.searchVideos = function(_interprete, _done) {
	var url = 'https://www.googleapis.com/youtube/v3/search?&maxResults=50&part=snippet&q=' + _interprete + '&key=' + apikey;
	console.log(url);
	var xhr = Ti.Network.createHTTPClient({
		onload : function() {
			if (this.status == 200) {
				...
				console.log(videos);
				var res = [];
				videos.items.forEach(function(v, i) {
					if (v.snippet && v.snippet.thumbnails)
						res.push({
							image : v.snippet.thumbnails.default.url.replace(/default/, 'hqdefault'),
							title : v.snippet.title.replace(/^[\s]+/g, ''),
							id : v.id,
