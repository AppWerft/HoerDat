...
//Rare exceptions to indicate a bug in the _hrdat code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_hrdat = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_hrdat]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.hrdat.module"
